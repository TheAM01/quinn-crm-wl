...
const ReferralLeaderboard = ({ campaign, leaderboardData: initialData, onClose }: Props) => {
    ...
    const fetchLeaderboard = async (page: number = 1): Promise<void> => {
        setLoading(true);
        try {
            const response = await fetch(`/api/campaigns/${campaign.campaign_id}/leaderboard?page=${page}`);
    ...
    const totalReferrals = leaderboard.reduce((sum, entry) => sum + entry.total_referrals, 0);
    const avgPointsPerReferrer = leaderboard.length > 0 ? (totalPoints / leaderboard.length).toFixed(1) : '0';
    ...
    return (
        <div className="flex flex-1 flex-col p-3 bg-yellow-50">
            <button
                onClick={onClose}
                className="p-2 w-min hover:shadow-sm rounded-full duration-200 items-center justify-center cursor-pointer flex text-black border border-neutral-400 transition-all"
            >
                <ChevronLeft size={14}/>
            </button>
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-xl shadow-lg border-t-4 border-yellow-500">
                        ...
                        <p className="text-3xl font-bold text-gray-900">{data.pagination?.total_entries || 0}</p>
                    ...
                    {(data.pagination?.total_pages || 0) > 1 && (
                        <div className="bg-gray-50 px-6 py-4 flex items-center justify-between border-t border-gray-200">
                            <div className="text-sm text-gray-600">
                                Showing page {data.pagination?.current_page || 1} of {data.pagination?.total_pages || 1}
                            </div>
