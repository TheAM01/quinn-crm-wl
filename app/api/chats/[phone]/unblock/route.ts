...
export async function POST(
    ...
    try {
        ...
        // Remove "blocked" tag from existing tags
        const existingTags = customerData.tags || [];
        const updatedTags = existingTags.filter((tag: string) => tag.toLowerCase() !== "blocked");

        // Update the customer data without the blocked tag
        const updatedData = {
            ...customerData,
            tags: updatedTags
        };
