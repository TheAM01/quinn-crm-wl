...
export async function POST(request: NextRequest) {
	try {
		...
		backendFormData.append('filename', file.name); // Use the actual filename
		backendFormData.append('author', "admin");

		const response = await fetch(`${SERVER_BASE_URL}/upload/documents`, {
			method: 'POST',
