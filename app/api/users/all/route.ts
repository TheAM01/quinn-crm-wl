...
interface CustomUserData extends CustomerData {
...
async function getOrderedCustomersWithStatus() {
	try {
		...
		customersData.customers.forEach((customer: CustomerData) => {
		...
		const customersWithoutChats = customersData.customers
			.filter((customer: CustomerData) => !chatPhoneNumbers.has(customer.phone_number))
